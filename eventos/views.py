...
def gerenciar_evento(request):
    if request.method == "GET":
        ...
        if nome:
            eventos = eventos.objects.filter(nome__contains=nome)  #pega os eventos que o usuarios logado criou
        print(eventos) 
...
# Validar inscrição
    ...
    if request.method == "GET":
        ...
        # Validar se o usuário é um participante
...
def participantes_evento(request, id):
    ...
    if not evento.criador == request.user:
        raise Http404('Este evento não é seu') #mensagem de erro
    if request.method == 'GET':
...
def gerar_csv(request, id):
    ...
    with open(path, 'w') as arq:
        write = csv.writer(arq, delimiter=",")
